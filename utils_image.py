...
def img_save(image,imagename,savepath):
    ...
    # Gray_pic
    imsave(os.path.join(savepath, "{}.png".format(imagename)),np.uint8(image))
...
def get_image_path_name(dataroot):
    paths = None  # return None if dataroot is None
    names=None
    if isinstance(dataroot, str):
        paths,names = sorted(_get_path_name_from_images(dataroot))
        paths=sorted(paths)
        names=sorted(names)
    elif isinstance(dataroot, list):
        paths = []
        names=[]
        for i in dataroot:
            p,n=_get_path_name_from_images(i)
            paths += sorted(p)
            names += sorted(n)
    return paths,names


def _get_path_name_from_images(path):
    assert os.path.isdir(path), '{:s} is not a valid directory'.format(path)
    images = []
    names=[]
    for dirpath, _, fnames in sorted(os.walk(path)):
...
def saveTensorImage(tensorI,save_dir,save_name):
    tensorI = tensorI.detach()[0].float().cpu()
    tensorI = tensor2uint(tensorI)
    save_name = os.path.join(save_dir, os.path.basename(save_name))
    imsave(tensorI, save_name)

def saveTensorImage2(tensorI,save_dir,save_name):
    tensorI = tensorI.detach().float().cpu()
    tensorI = tensor2uint(tensorI)
    save_name = os.path.join(save_dir, os.path.basename(save_name))
    imsave(tensorI, save_name)

def norm01(A):
    batch_size, height, width = A.shape
    AA = A.view(batch_size, -1)
